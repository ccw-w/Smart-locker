...
void OLED_DrawRoundedRectangle(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, uint8_t Radius, uint8_t IsFilled)
{
    int16_t i, j;
    ...
    {
        OLED_DrawPoint(i, Y); // 上边
        OLED_DrawPoint(i, Y + Height - 1); // 下边
    }
    for (i = Y + Radius; i < Y + Height - Radius; i++)
    {
        OLED_DrawPoint(X, i); // 左边
        OLED_DrawPoint(X + Width - 1, i); // 右边
    }
    ...
    for (i = 0; i <= Radius; i++)
    {
        for (j = 0; j <= Radius; j++)
        {
            ...
            {
                OLED_DrawPoint(X + Radius - i, Y + Radius - j); // 左上角
                OLED_DrawPoint(X + Width - Radius + i - 1, Y + Radius - j); // 右上角
                OLED_DrawPoint(X + Radius - i, Y + Height - Radius + j - 1); // 左下角
                OLED_DrawPoint(X + Width - Radius + i - 1, Y + Height - Radius + j - 1); // 右下角
            }
        }
    }
    ...
    if (IsFilled)
    {
        ...
        for (i = 0; i <= Radius; i++)
        {
            for (j = 0; j <= Radius; j++)
            {
                if (sqrt(pow(i, 2) + pow(j, 2)) <= Radius)
                {
                    for (int k = Y + Radius - j; k < Y + Height - Radius + j; k++)
                    {
                        OLED_DrawPoint(X + Radius - i, k); // 左侧
                        OLED_DrawPoint(X + Width - Radius + i - 1, k); // 右侧
                    }
                    for (int k = X + Radius - i; k < X + Width - Radius + i; k++)
                    {
                        OLED_DrawPoint(k, Y + Radius - j); // 上方
                        OLED_DrawPoint(k, Y + Height - Radius + j - 1); // 下方
                    }
                }
            }
        }
